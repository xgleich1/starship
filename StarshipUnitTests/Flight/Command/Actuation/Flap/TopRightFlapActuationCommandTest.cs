...
{
    public sealed class TopRightFlapActuationCommandTest
    {
        [Test]
        public void Should_clamp_and_expose_the_commanded_deploy_percent()
        {
            // GIVEN
            var commandA = new TopRightFlapActuationCommand(-0.1F);
            var commandB = new TopRightFlapActuationCommand(0.0F);
            var commandC = new TopRightFlapActuationCommand(0.5F);
            var commandD = new TopRightFlapActuationCommand(1.0F);
            var commandE = new TopRightFlapActuationCommand(1.1F);
        ...
        {
            // GIVEN
            var command = new TopRightFlapActuationCommand(1.1F);

            // THEN
            var expectedTelemetry = new List<TelemetryMessage>
            {
                new TelemetryMessage("TopRightFlapDeployPercent:1,1")
            };
        ...
        {
            // GIVEN
            var commandA = new TopRightFlapActuationCommand(0.5F);
            var commandB = new TopRightFlapActuationCommand(0.5F);
            var commandC = new TopRightFlapActuationCommand(1.0F);
