...
namespace StarshipUnitTests.Flight.Command.Throttle.Main
{
    public sealed class TopMainEngineThrottleCommandTest
    {
        ...
        {
            // GIVEN
            var commandA = new TopMainEngineThrottleCommand(-0.1F);
            var commandB = new TopMainEngineThrottleCommand(0.0F);
            var commandC = new TopMainEngineThrottleCommand(0.5F);
            var commandD = new TopMainEngineThrottleCommand(1.0F);
            var commandE = new TopMainEngineThrottleCommand(1.1F);
        ...
        {
            // GIVEN
            var command = new TopMainEngineThrottleCommand(1.1F);

            // THEN
            var expectedTelemetry = new List<TelemetryMessage>
            {
                new TelemetryMessage("TopMainEngineThrottlePercent:1,1")
            };
